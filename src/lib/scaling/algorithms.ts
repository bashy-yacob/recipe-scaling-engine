...
export function getDefaultScalingRule(ingredientName: string): ScalingRule['type'] {
  ...
  const logarithmicIngredients = ['yeast', 'שמרים', 'baking powder', 'אבקת אפייה', 'baking soda', 'סודה', 'chocolate', 'שוקולד', 'cocoa', 'קקאו'];
  if (logarithmicIngredients.some((ing) => name.includes(ing))) {
    return 'logarithmic';
  }

  // Square root: salt, spices, herbs
  const sqrtIngredients = ['salt', 'מלח', 'pepper', 'פלפל', 'spice', 'תבלין', 'herb', 'עשב', 'cinnamon', 'קינמון', 'cumin', 'כמון'];
  if (sqrtIngredients.some((ing) => name.includes(ing))) {
    return 'sqrt';
  }

  // Fixed: vanilla, food coloring, small flavor enhancers
  const fixedIngredients = ['vanilla', 'וניל', 'food coloring', 'צבע מאכל', 'extract', 'תמצית'];
  if (fixedIngredients.some((ing) => name.includes(ing))) {
    return 'fixed';
  }
...
export type { ScalingRule, ScaledResult };
